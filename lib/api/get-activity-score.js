...
async function getActivityScore(member) {
  ...
  const clanMemberIds = Object.keys(await getClanMembers());
  ...
  for (let i = 0; i < characters.length; i++) {
    ...
    activities.forEach(activity => {
      ...
      activity.players.forEach(player => {
        ...
        if (id !== member.id && clanMemberIds.includes(id)) {
          ...
          if (activityEntry.modes[activity.mode]) {
            activityEntry.modes[activity.mode]++;
          } else {
            activityEntry.modes[activity.mode] = 1;
          }
        }
      });
    });
