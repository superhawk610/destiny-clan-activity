...
async function getActivityScore(member) {
  ...
  for (let i = 0; i < characters.length; i++) {
    ...
    activities.forEach(activity => {
      ...
      activityEntry.completions[mode] = ++activityEntry.completions[mode] || 1;
      ...
      activity.players.forEach(player => {
        ...
        if (id !== member.id && clanMembers[id]) {
          ...
          activityEntry.modes[mode] = ++activityEntry.modes[mode] || 1;
        }
      });
    });
  }
  ...
  return {
    ...
    completions: activityEntry.completions,
